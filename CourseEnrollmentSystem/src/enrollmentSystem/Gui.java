...
public class Gui {
    ...
    JPanel studentPanelTable= new JPanel();
	...
	private void showStudents() throws Exception {
		...
		//para ma north ung mga shits
		JPanel tablestudentToNorth = new JPanel();
		
		tableStudent.setBackground(Color.gray);
        GridBagConstraints gbc = new GridBagConstraints();
        
        
        JScrollPane studentScrollPane= new JScrollPane(tableStudent);
        
        tablestudentToNorth.add(studentScrollPane,BorderLayout.NORTH);
        studentCanvas.add(studentScrollPane,BorderLayout.CENTER);
		// COLUMN ITO
        ...
        for (int i = 0; i < columnNames.length; i++) {
            ...
            gbc.weightx = 1.0;
            gbc.weighty = 0.1;
            JLabel columnStart = new JLabel(columnNames[i], JLabel.CENTER);
            columnStart.setFont(new Font("Arial", Font.BOLD, 25));
            columnStart.setBorder(new EmptyBorder(2,2,2,2));
            tableStudent.add(columnStart, gbc);
        }
        ...
        for (int row = 0; row < data.length; row++) {
            ...
            for (int col = 0; col < data[row].length; col++) {
                ...
                //DITO UNG MGA STUDENTS
                JLabel studentDataLabel = new JLabel(data[row][col], JLabel.CENTER);
                
                
                //DITO UNG MGA DESIGN
                Dimension studentlabelDimension = new Dimension(50,50);
                studentDataLabel.setMaximumSize(studentlabelDimension);
                studentDataLabel.setBorder(new EmptyBorder(5,5,5,5));
                studentDataLabel.setBorder(BorderFactory.createLineBorder(Color.BLACK));
                tableStudent.add(studentDataLabel, gbc);
            }
        }
	...
	private void showStudentTableForm() {
		studentPanelTable.removeAll();
		
		studentPanelTable.setBackground(Color.decode("#013d21"));
        ...
        studentPanelTable.setPreferredSize(new Dimension(900,650));
	...
	private void rightPanelFunctions() {
     ...
     // Add all panels to the rightPanel (CardLayout)
        ...
        rightPanel.add(studentPanelTable,"studentPanelTable");
